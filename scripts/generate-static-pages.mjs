...
const page = ({ lang, path, title, description, body }) => { const localizedPath = path.replace(/^en\//, ''); return `<!DOCTYPE html><html lang="${lang}"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><meta name="description" content="${escape(description)}"><link rel="canonical" href="${siteUrl}${path}"><link rel="alternate" hreflang="pt-BR" href="${siteUrl}${localizedPath}"><link rel="alternate" hreflang="en" href="${siteUrl}en/${localizedPath}"><meta property="og:type" content="website"><meta property="og:url" content="${siteUrl}${path}"><meta property="og:title" content="${escape(title)}"><meta property="og:description" content="${escape(description)}"><meta property="og:image" content="${siteUrl}og-image.png"><title>${escape(title)}</title></head><body><div id="root"><main><h1>${escape(title)}</h1><p>${escape(description)}</p>${body}</main></div>${globalThis.appScript}</body></html>` }
